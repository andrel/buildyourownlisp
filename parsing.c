...
// Enumeration type to string representation
...
/* Create pointer to a builtin function pointer */
...
void lenv_put(lenv *e, lval *k, lval *v) {
  ...
  /* No existing entry: allocate space for new entry */
  e->count++;
...
lval *lval_read_str(mpc_ast_t *t) {
  ...
  // String last quote character
  t->contents[strlen(t->contents) - 1] = '\0';

  // Copy the string, skip first quote character
...
lval* builtin_op(__attribute__ ((unused)) lenv *e, lval *a, char *op) {
...
lval *builtin_load(lenv *e, lval *a) {
  ...
  if (mpc_parse_contents(a->cell[0]->str, Lispy, &r)) {
      ...
      // Use parse error to create new error message
...
lval* builtin_var(lenv *e, lval *a, char *func) {
  ...
  // Check correct number of symbol of values
  LASSERT(a, (syms->count == a->count - 1),
          "Function 'def' cannot define incorrect number of values to symbols", NULL);
...
lval* builtin_lambda(__attribute__ ((unused)) lenv *e, lval *a) {
  ...
  // Check two arguments. Both are Q-Expressions.
  LASSERT_NUM("\\", a, 2);
  LASSERT_TYPE("\\", a, 0, LVAL_QEXPR);
  LASSERT_TYPE("\\", a, 1, LVAL_QEXPR);

  // Check that first Q-Expression only Symbols.
...
int main(int argc, char **argv) {
  ...
  mpc_cleanup(8, String, Comment, Number, Symbol, Sexpr, Qexpr, Expr, Lispy);
